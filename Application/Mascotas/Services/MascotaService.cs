    ...
    public class MascotaService : IMascotaService
    {
        ...
        public async Task<int> CrearAsync(MascotaRequestDto mascotaDto)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(mascotaDto.TipoAnimal))
                ...
                if (!Enum.TryParse<TipoAnimalE>(mascotaDto.TipoAnimal, true, out var AnimalIdEnum))
                    throw new ArgumentException("Género inválido.");
        ...
        public async Task ActualizarAsync(int id, bool aprobado, bool disponible)
        {
            ...
            await _mascotaRepository.ActualizarAsync(id, aprobado, disponible);
        }

        public async Task EliminarAsync(int id)
        {
            await _mascotaRepository.EliminarAsync(id);
        }
        ...
        public async Task<MascotaInfoDto?> ObtenerPorIdUserAsync(int id)
        {
        ...
        public async Task<IEnumerable<MascotaPreviewDto>> ObtenerTodasUserAsync(FiltrosRequestDTO filtros)
        {
            try
            {
                var consulta = await _mascotaRepository.GetMascotasUserAsync(filtros);
        ...
        public async Task<MascotaDetallesAdminDto?> ObtenerPorIdAdminAsync(int id)
        {
            var mascota = await _mascotaRepository.GetMascotaAdminByIdAsync(id);
        ...
        public async Task<List<MascotaPreviewAdminDto>> ObtenerTodasAdminAsync()
        {
            try
            {
                var consulta = await _mascotaRepository.GetMascotasAdminAsync();
